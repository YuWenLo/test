    ...
    def __init__(self, num_layers, heads, pretrained, down_ratio, final_kernel,
        ...
        if num_layers == 85:
          ...
          if pretrained:
            ...
            self.last_proj = ConvLayer(328, 128, kernel=1)
            self.last_blk = HarDBlock(384, 64, 1.7, 8)
            self.skip_nodes = [1,3,8]
            self.SC = [32, 0]  
            gr = [32, 20, 16]
            layers = [8, 8, 4]
            ch_list2 = [192+self.SC[0], 64+self.SC[1]]
            channels = [64, 124, 328]
            self.skip_lv = 1
            scales = [2 ** i for i in range(len(channels[self.first_level:]))]
